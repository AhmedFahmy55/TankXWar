...
public class MatchplayBackfiller : IDisposable
{
    ...
    public void AddPlayerToMatch(PlayerData playerData)
    {
        if (!IsBackfilling)
        {
        ...
        if (GetPlayerById(playerData.playerAuthID) != null)
        {
            Debug.LogWarningFormat("User: {0} - {1} already in Match. Ignoring add.",
                playerData.playerName,
                playerData.playerAuthID);
                
            return;
        }

        Player matchmakerPlayer = new Player(playerData.playerAuthID, playerData.PlayerGamePreferences);
