...
public class MatchplayMatchmaker : IDisposable
{
    ...
    public async Task<MatchmakingResult> Matchmake(PlayerData data)
    {
        cancelToken = new CancellationTokenSource();

        string queueName = data.PlayerGamePreferences.ToMultiplayQueue();
        CreateTicketOptions createTicketOptions = new CreateTicketOptions(queueName);
        Debug.Log(createTicketOptions.QueueName);

        List<Player> players = new List<Player>
        {
            new Player(data.playerAuthID, data.PlayerGamePreferences)
        };
